...
# --- UI ---
t1, t2, t3, t4 = st.tabs(["📥 Vložit", "🔮 Predikce", "🏆 Žebříček", "⚙️ Správa & Historie"])

with t1:
    raw_in = st.text_area("Vložte text zápasu:", height=200)
...
with t2:
    ratings = get_ratings()
    c1, c2 = st.columns(2)
    pA = c1.text_input("Hráč A").upper(); pB = c2.text_input("Hráč B").upper()
    col1, col2, col3 = st.columns(3)
    s_side = col1.radio("Podává:", ["Hráč A", "Hráč B"])
    kA = col2.number_input("Kurz A", 1.01, 10.0, 1.85)
...
with t4:
    ...
    if st.session_state.data:
        df_hist = pd.DataFrame(st.session_state.data)
        for i, row in df_hist[::-1].iterrows():
            c1, c2, c3 = st.columns([0.6, 0.2, 0.2])
            c1.write(f"**{row['timestamp']}** | {row['A']} vs {row['B']} | {row['score']}")
            if c3.button("Smazat", key=f"del_{i}"):
