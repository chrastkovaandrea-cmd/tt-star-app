...
# --- 4. UI ---
st.set_page_config(page_title="TT STAR v12.6 ULTRA", layout="wide")
st.title("🏓 TT STAR - ULTRA ANALYTIK")

t1, t2, t3, t4 = st.tabs(["📥 Vložit Set", "🔮 Predikce & Value", "🏆 Žebříček", "⚙️ Historie"])

with t1:
    with st.expander("🆘 Záchrana dat (Vložení historie)"):
        import_txt = st.text_area("Sem vlož zkopírovaný JSON text z včerejší historie:")
        if st.button("♻️ Obnovit všechna data"):
            try:
                imported_data = json.loads(import_txt)
                st.session_state.data = imported_data
                save_data(imported_data)
                st.success("Hotovo! Data jsou zpět.")
                st.rerun()
            except: st.error("Chyba: Neplatný formát dat.")
    ...
    current_starter = m_first_match if m_set % 2 != 0 else ("B" if m_first_match == "A" else "A")
...
with t2:
    ...
    if pA_in and pB_in:
        ...
        v1, v2 = st.columns(2)
        with v1: st.metric(f"Vítěz {pA_in}", f"{round(pA_w*100,1)}%", f"Fair: {round(1/pA_w,2)}")
        with v2: st.metric("Over 18.5 bodů", f"{round(p_ov*100,1)}%", f"Fair: {round(1/p_ov,2)}")
...
with t4:
    ...
    st.download_button(label="📥 STÁHNOUT ZÁLOHU DO MOBILU", data=json.dumps(st.session_state.data, indent=4), file_name="tt_backup.json")
