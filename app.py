...
# --- CONFIG ---
st.set_page_config(page_title="TT STAR ANALYTIK v18.2 PRO", page_icon="🏓", layout="wide")

# Připojení ke Google Sheets
# POZOR: Vyžaduje mít v GitHubu soubor requirements.txt s obsahem: streamlit-gsheets
try:
    conn = st.connection("gsheets", type=GSheetsConnection)
except Exception as e:
    st.error("Chyba připojení ke Google Sheets. Zkontroluj Secrets a requirements.txt.")
    st.stop()
...
def load_data():
    try:
        # ttl=0 zajistí, že se data pokaždé načtou znovu z tabulky bez mezipaměti
        df = conn.read(worksheet="Data", ttl=0)
        # Odstraníme prázdné řádky, pokud v tabulce jsou
...
def save_data(data_list):
    ...
    # Po uložení vymažeme cache, aby aplikace věděla, že jsou nová data
    st.cache_data.clear()
...
# --- GLICKO-2 LOGIKA (S časovou váhou a body) ---
def get_ratings():
    ratings = {}
    BASE_R, BASE_RD = 1500, 350
    # Seřazení dat podle času pro správný výpočet formy
...
with t1:
    st.subheader("Nový zápas do Google Cloudu")
    raw_in = st.text_area("Vlož text od Gemini:", placeholder="Jméno A - Jméno B | 3:0 (11:5, 11:5, 11:5) | Kurzy: 1.50, 2.50")
    m_first = st.selectbox("Kdo podával v 1. SETU?", ["Hráč 1 (Horní)", "Hráč 2 (Dolní)"])
    if st.button("🚀 ULOŽIT A SYNCHRONIZOVAT"):
        try:
            parts = raw_in.split('|')
            names = parts[0].split('-')
            ...
            st.success("Data byla zapsána do Google Tabulky!"); st.rerun()
        except: st.error("Chyba formátu. Zkontroluj text.")
...
with t2:
    ...
    k_win = col_a.number_input("Kurz Výhra A", 1.01, 20.0, 1.85)
    k_over = col_b.number_input("Kurz Over 18.5", 1.01, 20.0, 1.85)
    live_s = col_c.radio("Podává právě teď:", ["Hráč A", "Hráč B"])
    ...
    if selA and selB and selA != selB:
        ...
        # Live úprava podle podání
        probA = min(max(probA + (0.06 if live_s == "Hráč A" else -0.06), 0.01), 0.99)
        
        st.metric(f"Pravděpodobnost {selA}", f"{round(probA*100,1)}%")
        if (probA * k_win) > 1.05: st.success(f"🔥 VALUE VÝHRA: +{round(((probA*k_win)-1)*100,1)}%")
        
        # Šance na Over 18.5 přes simulaci setů
        ...
        st.write(f"**Šance na Over 18.5:** {round(pOver*100,1)}%")
        if (pOver * k_over) > 1.05: st.success(f"🔥 VALUE OVER: +{round(((pOver*k_over)-1)*100,1)}%")
        st.info(f"Odhad bodů v setu: {11 if probA > 0.5 else round(11*(probA/0.5))} : {11 if probA < 0.5 else round(11*((1-probA)/0.5))}")

with t3:
    ratings = get_ratings(); search = st.text_input("🔍 Vyhledat hráče")
    if ratings:
...
with t4:
    st.subheader("⚙️ Správa dat v Cloudu")
    if st.button("🔄 REFRESH (Načíst znovu z tabulky)"):
        st.session_state.data = load_data(); st.rerun()
    
    up = st.file_uploader("Nahrát CSV zálohu do Cloudu (přepíše tabulku!)", type="csv")
    if up and st.button("⬆️ POTVRDIT NAHRÁNÍ"):
        st.session_state.data = pd.read_csv(up).to_dict('records')
        save_data(st.session_state.data); st.success("Data nahrána do Google Sheets!"); st.rerun()

    st.write("---")
    # Detailní editace historie
    for i, row in enumerate(st.session_state.data[::-1]):
        idx = len(st.session_state.data) - 1 - i
        with st.expander(f"{row['A']} vs {row['B']} | {row['score']} | 🎾 {row['A'] if row['starter']=='A' else row['B']}"):
            c1, c2, c3 = st.columns(3)
            eA = c1.text_input("Hráč A", row['A'], key=f"a{idx}"); eB = c2.text_input("Hráč B", row['B'], key=f"b{idx}"); eS = c3.text_input("Skóre", row['score'], key=f"s{idx}")
            eSt = st.selectbox("Podával:", [eA, eB], index=0 if row['starter']=="A" else 1, key=f"st{idx}")
            if st.button("Uložit změny", key=f"sv{idx}"):
                st.session_state.data[idx].update({"A": eA.upper(), "B": eB.upper(), "score": eS, "starter": "A" if eSt == eA else "B"})
                save_data(st.session_state.data); st.rerun()
            if st.button("Smazat řádek", key=f"dl{idx}"):
